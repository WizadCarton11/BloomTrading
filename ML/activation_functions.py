...
class ActivationClass:
    ...
    def relu_activation_function(self, x: np.ndarray) -> np.ndarray:
        try:
    ...
    @typechecked(collection_check_strategy=CollectionCheckStrategy.ALL_ITEMS)
    def soft_max_activation_function(self, x: np.ndarray) -> np.ndarray:
        try:
    ...
    @typechecked(collection_check_strategy=CollectionCheckStrategy.ALL_ITEMS)
    def sigmoid_activation_function(self, x: np.ndarray) -> np.ndarray:
        try:
            return 1 / (1 + np.exp(-x))
        except Exception as e:
    ...
    @typechecked(collection_check_strategy=CollectionCheckStrategy.ALL_ITEMS)
    def tanh_activation_function(self, x: np.ndarray) -> np.ndarray:
        try:
            return np.tanh(x)
        except Exception as e:
    ...
    @typechecked(collection_check_strategy=CollectionCheckStrategy.ALL_ITEMS)
    def leaky_relu_activation_function(self, x: np.ndarray, alpha: float = 0.01) -> np.ndarray:
        try:
    ...
    @typechecked(collection_check_strategy=CollectionCheckStrategy.ALL_ITEMS)
    def elu_activation_function(self, x: np.ndarray, alpha: float = 1.0) -> np.ndarray:
        try:
