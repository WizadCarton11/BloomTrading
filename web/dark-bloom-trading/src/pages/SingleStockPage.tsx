...
const ValueComponent = ({ symbol }: ValueComponentProps) => {
  ...
  return (
    ...
    {data?
      ...
      <div
        className={`flex items-center gap-1 transition-all duration-300 ${ data?.change >= 0
...
const SingleStockPage = () => {
  ...
  const symbol = window.location.pathname.split("/").pop().toUpperCase() || "AAPL"; // Get the symbol from the URL path
  console.log("Rendering SingleStockPage for symbol:", symbol);
  useEffect(() => {
    const fetchStockData = async () => {
      try {
        const response = await MarketService.get(`/api/stock/stock_details/${symbol}`, {}, `get${symbol}StockDetails`);
        ...
        if (result) {
          ...
          result?.historicalData?.forEach((item: any) => {
            stockPriceData.push({ date: item.date, price: item.high,
  ...
  if (!stockData || Object.keys(stockData).length === 0 || todayData.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900 text-white">
        <div className="text-center">
          <h1 className="text-4xl font-bold mb-4">Loading Stock Data...</h1>
          <p className="text-lg text-gray-400">Please wait while we fetch the latest stock information.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="min-h-screen bg-gray-950 text-white p-4 md:p-6">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <Card className="bg-gray-900 border-gray-800 hover:border-blue-800/50 transition-all duration-300 hover:shadow-lg hover:shadow-blue-900/20">
              ...
              <CardContent>
                <StockChart historicalData={historicalData} symbol={stockData?.symbol} latestDate={latestDate} 
                //  todayData={todayData}
                 />
              </CardContent>
            </Card>
